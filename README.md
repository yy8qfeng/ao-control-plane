...
- 上层软件负责调用 Codex 拆解结构化任务计划，并调用 ClaudeCode 审查任务计划。
...
如果 ClaudeCode 设计审查输出不是合法 JSON，系统会先尝试自动修复；仍无法修复时，会把原始审查文本作为未解决意见纳入整改，避免设计审查流程因为格式问题直接中断。任务计划阶段由 Codex 输出 `task-plan` JSON，并由 ClaudeCode 输出结构化任务计划审查 JSON；ClaudeCode 审查 JSON 最终仍无法通过 schema 校验时，会写入 `invalid-claude-output.txt` 和 `human-review-required.json`，需要人工复核后再继续。Codex 生成的 `task-plan` JSON 如果无法通过 schema 校验，workflow 会标记为 `failed`，错误信息会进入流程日志和 `workflow.json`，不会伪装为 ClaudeCode 审查产物。
